...
import { VideoSectionRespond } from "@/types/videoSection";
...
async function fetchVideoSections(): Promise<VideoSection[]> {
  const url = `${apiBase()}/video-sections`;
  try {
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return [];
    const json = await res.json();
...
export default async function HomePage() {
  const [news, latestEvent, videosRaw, videoSectionsRaw] = await Promise.all([
    getNewsPreview(),
    getLatestEvent(),
    fetchVideos(),
    fetchVideoSections(),
  ]);
  console.log("videoSectionsRaw", videoSectionsRaw);
  ...
  const videoSections: VideoSectionRespond[] = (videoSectionsRaw || [])
    .map((p: any) => {
      return {
        ...p,
        video_path: normalizeVideoUrl(p.video_path) || p.video_path,
      } as VideoSectionRespond;
    })
    .filter((p) => !!p.video_path);
      ...
      <HomeVerticalSwiper>
        ...
        >
          <div className="relative h-full w-full flex items-start">
            ...
            <ScaleToFit className="w-full h-full pt-16 pb-2">
              <div className="relative w-full bg-transparent p-4 min-w-[1440px]">
                <HomeVideoShowcase videos={videoSections} />
              </div>
            </ScaleToFit>
          </div>
        </section>
